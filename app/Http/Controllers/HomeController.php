...
class HomeController extends Controller
{
    public function home(Request $request)
    {
        ...
        } else if ($request->user()->hasRole('Assigned')) {
            $params['user_id'] = auth()->user()->id;
            $params['ATasks'] = Task::where('user_id', $params['user_id'])->get();
            return view('Dashboard.Assigned.Employee', $params);
        } else {
            $params['user_id'] = auth()->user()->id;
            $params['ATasks'] = Task::where('user_id', $params['user_id'])->get();
            return view('Dashboard.Admin.index', $params);
        }
    }
...
//        dd($params['febCompleteTask']);
    ...
    public function TasksAssign()
    {
        ...
        $params['MyTasks'] = Task::where('created_by', $user_id)->get()->count();
        $params['done'] = Task::where([['created_by', $user_id], ['status', 1]])->get()->count();
        $params['notdone'] = Task::where([['created_by', $user_id], ['status', 0]])->get()->count();
